...
describe('@mantine-datepicker-jalali/JalaliYearPicker', () => {
  itSupportsYearsListProps(JalaliYearPicker, defaultProps);
  ...
  it('can be uncontrolled (type="default")', async () => {
    const { container } = render(
      <JalaliYearPicker {...defaultProps} date={new Date(2022, 3, 11)} />
    );
  ...
  it('can be uncontrolled (type="multiple")', async () => {
    const { container } = render(
      <JalaliYearPicker {...defaultProps} type="multiple" date={new Date(2022, 3, 11)} />
    );
  ...
  it('can be uncontrolled (type="range")', async () => {
    const { container } = render(<JalaliYearPicker {...defaultProps} type="range" />);
    expect(container.querySelectorAll('[data-selected]')).toHaveLength(0);
  ...
  it('can be controlled (type="range")', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliYearPicker {...defaultProps} type="range" value={[null, null]} onChange={spy} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('supports onClick handler from getYearControlProps', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliYearPicker {...defaultProps} getYearControlProps={() => ({ onClick: spy })} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('supports allowDeselect', async () => {
    const spy = jest.fn();
    const { container, rerender } = render(<JalaliYearPicker {...defaultProps} onChange={spy} />);
    ...
    rerender(<JalaliYearPicker {...defaultProps} onChange={spy} allowDeselect />);
    await userEvent.click(container.querySelector('table button'));
  ...
  it('handles allowSingleDateInRange={true} correctly', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliYearPicker {...defaultProps} type="range" allowSingleDateInRange onChange={spy} />
    );
  ...
  it('has correct default __staticSelector', () => {
    const { container } = render(<JalaliYearPicker {...defaultProps} />);
    expect(container.firstChild).toHaveClass('mantine-YearPicker-calendar');
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(
      <JalaliYearPicker {...defaultProps} __staticSelector="Calendar" />
    );
