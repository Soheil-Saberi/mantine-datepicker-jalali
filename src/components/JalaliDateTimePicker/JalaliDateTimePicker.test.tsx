...
import { JalaliDatesProvider } from '../JalaliDatesProvider';
import {
...
import { JalaliDateTimePicker, JalaliDateTimePickerProps } from './JalaliDateTimePicker';

const defaultProps: JalaliDateTimePickerProps = {
  popoverProps: { withinPortal: false, transitionDuration: 0 },
  modalProps: { withinPortal: false, transitionDuration: 0 },
  JalalitimeInputProps: { 'aria-label': 'test-time-input' },
...
describe('@mantine-datepicker-jalali/JalaliDateTimePicker', () => {
  itSupportsClearableProps(JalaliDateTimePicker, { ...defaultProps, defaultValue: new Date() });
  itSupportsYearsListProps(JalaliDateTimePicker, {
    ...defaultProps,
    defaultLevel: 'decade',
    defaultValue: new Date(),
    popoverProps: { opened: true, withinPortal: false, transitionDuration: 0 },
  });
  itSupportsMonthsListProps(JalaliDateTimePicker, {
    ...defaultProps,
    defaultLevel: 'year',
    defaultValue: new Date(),
    popoverProps: { opened: true, withinPortal: false, transitionDuration: 0 },
  ...
  it('toggles popover when input is clicked (dropdownType="popover")', async () => {
    const { container } = render(<JalaliDateTimePicker {...defaultProps} />);
  ...
  it('toggles modal when input is clicked (dropdownType="modal")', async () => {
    const { container } = render(<JalaliDateTimePicker {...defaultProps} dropdownType="modal" />);
  ...
  it('closes dropdown when user presses Enter in TimeInput', async () => {
    const { container } = render(
      <JalaliDateTimePicker {...defaultProps} defaultValue={new Date(2022, 3, 11)} />
    );
    await clickInput(container);
    expectOpenedPopover(container);
  ...
  it('supports controlled state', async () => {
    ...
    const { container } = render(
      <JalaliDateTimePicker {...defaultProps} value={new Date(2022, 3, 11)} onChange={spy} />
    );
  ...
  it('renders clear button based on clearable prop and current value', () => {
    const { rerender } = render(
      <JalaliDateTimePicker {...defaultProps} value={new Date(2022, 3, 11)} clearable />
    );

    expect(getClearButton()).toBeInTheDocument();

    rerender(
      <JalaliDateTimePicker {...defaultProps} value={new Date(2022, 3, 11)} clearable={false} />
    );
    expect(getClearButton()).toBe(undefined);

    rerender(<JalaliDateTimePicker {...defaultProps} value={null} clearable />);
    expect(getClearButton()).toBe(undefined);
  });

  it('clears input when clear button is clicked', async () => {
    const { container } = render(
      <JalaliDateTimePicker {...defaultProps} defaultValue={new Date(2022, 3, 11)} clearable />
    );
  ...
  it('does not open popover if readOnly prop is set', async () => {
    const { container } = render(<JalaliDateTimePicker {...defaultProps} readOnly />);
    expectNoPopover(container);
    await clickInput(container);
    expectNoPopover(container);
  });
