...
import { JalaliWeekdaysRow, WeekdaysRowStylesNames } from '../JalaliWeekdaysRow';
import { JalaliDay, DayStylesNames, JalaliDayProps } from '../JalaliDay';
...
import useStyles from './JalaliMonth.styles';
...
export interface MonthSettings {
  ...
  getDayProps?(date: Date): Partial<JalaliDayProps>;
...
export interface JalaliMonthProps
...
const defaultProps: Partial<JalaliMonthProps> = {};

export const JalaliMonth = forwardRef<HTMLTableElement, JalaliMonthProps>((props, ref) => {
  const {
    className,
    classNames,
    styles,
  ...
  const rows = getMonthDays(month, ctx.getFirstDayOfWeek(firstDayOfWeek), locale).map(
    (row, rowIndex) => {
      const cells = row.map((date, cellIndex) => {
        const outside = !isSameMonth(date, month, locale);
        const ariaLabel =
...
JalaliMonth.displayName = '@mantine-datepicker-jalali/JalaliMonth';
