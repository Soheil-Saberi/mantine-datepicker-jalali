...
import useStyles from './JalaliMonthsList.styles';
...
export interface MonthsListSettings extends ControlsGroupSettings {
  ...
  getMonthControlProps?(date: Date): Partial<JalaliPickerControlProps>;
}

export interface JalaliMonthsListProps
...
const defaultProps: Partial<JalaliMonthsListProps> = {
  monthsListFormat: 'MMM',
};

export const JalaliMonthsList = forwardRef<HTMLTableElement, JalaliMonthsListProps>(
  (props, ref) => {
    const {
      year,
      className,
    ...
    const months =
      locale && locale === 'fa' ? moveArr(getMonthsData(year), 0, 3) : getMonthsData(year);
...
JalaliMonthsList.displayName = '@mantine-datepicker-jalali/JalaliMonthsList';
