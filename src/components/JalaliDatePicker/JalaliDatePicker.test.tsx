...
describe('@mantine-datepicker-jalali/JalaliDatePicker', () => {
  itSupportsYearsListProps(JalaliDatePicker, { ...defaultProps, defaultLevel: 'decade' });
  itSupportsMonthsListProps(JalaliDatePicker, { ...defaultProps, defaultLevel: 'year' });
  itHandlesMonthKeyboardEvents(JalaliDatePicker, defaultProps);
  itSupportsMonthProps(JalaliDatePicker, defaultProps);

  it('can be uncontrolled (type="default")', async () => {
    const { container } = render(
      <JalaliDatePicker {...defaultProps} date={new Date(2022, 3, 11)} />
    );
  ...
  it('can be uncontrolled (type="multiple")', async () => {
    const { container } = render(
      <JalaliDatePicker {...defaultProps} type="multiple" date={new Date(2022, 3, 11)} />
    );
  ...
  it('can be uncontrolled (type="range")', async () => {
    const { container } = render(<JalaliDatePicker {...defaultProps} type="range" />);
    expect(container.querySelectorAll('[data-selected]')).toHaveLength(0);
  ...
  it('can be controlled (type="range")', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliDatePicker {...defaultProps} type="range" value={[null, null]} onChange={spy} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('supports onClick handler from getDayProps', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliDatePicker {...defaultProps} getDayProps={() => ({ onClick: spy })} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('supports allowDeselect', async () => {
    const spy = jest.fn();
    const { container, rerender } = render(<JalaliDatePicker {...defaultProps} onChange={spy} />);
    ...
    rerender(<JalaliDatePicker {...defaultProps} onChange={spy} allowDeselect />);
    await userEvent.click(container.querySelector('table button'));
  ...
  it('handles allowSingleDateInRange={true} correctly', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliDatePicker {...defaultProps} type="range" allowSingleDateInRange onChange={spy} />
    );
  ...
  it('has correct default __staticSelector', () => {
    const { container } = render(<JalaliDatePicker {...defaultProps} />);
    expect(container.firstChild).toHaveClass('mantine-DatePicker-calendar');
  });

  it('supports custom __staticSelector', () => {
    const { container } = render(
      <JalaliDatePicker {...defaultProps} __staticSelector="Calendar" />
    );
