...
import { JalaliYearLevelGroup, JalaliYearLevelGroupProps } from './JalaliYearLevelGroup';
import {
...
const defaultProps: JalaliYearLevelGroupProps = {
...
describe('@mantine-datepicker-jalali/JalaliYearLevelGroup', () => {
  itSupportsMonthsListProps(JalaliYearLevelGroup, defaultProps);
  itSupportsHeaderProps(JalaliYearLevelGroup, defaultProps);
  itSupportsOnControlClick(JalaliYearLevelGroup, defaultProps);
  itSupportsOnControlMouseEnter(JalaliYearLevelGroup, defaultProps);
  itHandlesControlsKeyboardEvents(
    JalaliYearLevelGroup,
    'year',
  ...
  it('renders correct number of columns based on numberOfColumns prop', () => {
    const { rerender } = render(<JalaliYearLevelGroup {...defaultProps} numberOfColumns={1} />);
    expect(screen.getAllByLabelText('level-control')).toHaveLength(1);

    rerender(<JalaliYearLevelGroup {...defaultProps} numberOfColumns={2} />);
    expect(screen.getAllByLabelText('level-control')).toHaveLength(2);

    rerender(<JalaliYearLevelGroup {...defaultProps} numberOfColumns={3} />);
    expect(screen.getAllByLabelText('level-control')).toHaveLength(3);
  });

  it('renders correct years group based on year prop', () => {
    render(<JalaliYearLevelGroup {...defaultProps} numberOfColumns={3} />);
  ...
  it('supports levelControlAriaLabel as string', () => {
    render(<JalaliYearLevelGroup {...defaultProps} levelControlAriaLabel="test-label" />);
  ...
  it('has correct default __staticSelector', () => {
    const { container } = render(<JalaliYearLevelGroup {...defaultProps} />);
  ...
  it('supports custom __staticSelector', () => {
    const { container } = render(
      <JalaliYearLevelGroup {...defaultProps} __staticSelector="Calendar" />
    );
