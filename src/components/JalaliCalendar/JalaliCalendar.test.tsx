...
import { JalaliCalendar, JalaliCalendarProps } from './JalaliCalendar';
import {
...
const defaultProps: JalaliCalendarProps = {
...
describe('@mantine-datepicker-jalali/JalaliCalendar', () => {
  itSupportsMonthProps(JalaliCalendar, defaultProps);
  itHandlesMonthKeyboardEvents(JalaliCalendar, defaultProps);
  itHandlesControlsKeyboardEvents(JalaliCalendar, 'year', '.mantine-MonthsList-monthsList', {
    ...defaultProps,
    level: 'year',
  });
  itHandlesControlsKeyboardEvents(JalaliCalendar, 'decade', '.mantine-YearsList-yearsList', {
    ...defaultProps,
    level: 'decade',
  });
  ...
  it('sets correct aria-labels based on ariaLabels prop', () => {
    ...
    const { rerender } = render(
      <JalaliCalendar {...defaultProps} ariaLabels={testLabels} level="month" />
    );
    ...
    rerender(<JalaliCalendar {...defaultProps} ariaLabels={testLabels} level="year" />);
    ...
    rerender(<JalaliCalendar {...defaultProps} ariaLabels={testLabels} level="decade" />);
  ...
  it('supports numberOfColumns', () => {
    const { rerender } = render(
      <JalaliCalendar {...defaultProps} numberOfColumns={1} level="month" />
    );
    expectLevelsCount([1, 0, 0]);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={2} level="month" />);
    expectLevelsCount([2, 0, 0]);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={1} level="year" />);
    expectLevelsCount([0, 1, 0]);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={2} level="year" />);
    expectLevelsCount([0, 2, 0]);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={1} level="decade" />);
    expectLevelsCount([0, 0, 1]);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={2} level="decade" />);
    expectLevelsCount([0, 0, 2]);
  });

  it('changes level correctly', async () => {
    const { container } = render(<JalaliCalendar {...defaultProps} />);
    expectLevelsCount([1, 0, 0]);
  ...
  it('supports defaultLevel prop (uncontrolled)', async () => {
    render(<JalaliCalendar {...defaultProps} defaultLevel="year" />);
    expectLevelsCount([0, 1, 0]);
    await userEvent.click(screen.getByLabelText('year-level'));
    expectLevelsCount([0, 0, 1]);
  });
  ...
  it('supports level prop (controlled)', async () => {
    render(<JalaliCalendar {...defaultProps} level="year" />);
    expectLevelsCount([0, 1, 0]);
    await userEvent.click(screen.getByLabelText('year-level'));
    expectLevelsCount([0, 1, 0]);
  });
  ...
  it('calls onLevelChange when level changes', async () => {
    const spy = jest.fn();
    const { container } = render(<JalaliCalendar {...defaultProps} onLevelChange={spy} />);
  ...
  it('renders correct header labels with defaultDate (uncontrolled)', async () => {
    render(<JalaliCalendar {...defaultProps} defaultDate={new Date(2021, 3, 11)} />);
  ...
  it('renders correct header labels with date (controlled)', async () => {
    render(<JalaliCalendar {...defaultProps} date={new Date(2021, 3, 11)} />);
    expectHeaderLevel('month', 'April 2021');
  ...
  it('changes displayed date when next/previous controls are clicked with defaultDate prop (uncontrolled)', async () => {
    const { rerender } = render(<JalaliCalendar {...defaultProps} level="month" />);
    ...
    rerender(<JalaliCalendar {...defaultProps} level="year" />);
    ...
    rerender(<JalaliCalendar {...defaultProps} level="decade" />);
    expectHeaderLevel('decade', '2020 – 2029');
  ...
  it('does not change date when next/previous controls are clicked with date prop (controlled)', async () => {
    const { rerender } = render(
      <JalaliCalendar {...defaultProps} date={new Date(2022, 3, 11)} level="month" />
    );
    expectHeaderLevel('month', 'April 2022');
    await clickNext('month');
    expectHeaderLevel('month', 'April 2022');

    rerender(<JalaliCalendar {...defaultProps} date={new Date(2022, 3, 11)} level="year" />);
    expectHeaderLevel('year', '2022');
    await clickNext('year');
    expectHeaderLevel('year', '2022');

    rerender(<JalaliCalendar {...defaultProps} date={new Date(2022, 3, 11)} level="decade" />);
    expectHeaderLevel('decade', '2020 – 2029');
    await clickNext('decade');
    expectHeaderLevel('decade', '2020 – 2029');
  });
  ...
  it('supports maxLevel', async () => {
    render(<JalaliCalendar {...defaultProps} defaultLevel="month" maxLevel="year" />);
    expectLevelsCount([1, 0, 0]);
    await userEvent.click(screen.getByLabelText('month-level'));
    expectLevelsCount([0, 1, 0]);
    await userEvent.click(screen.getByLabelText('year-level'));
  ...
  it('supports minLevel', async () => {
    const { container } = render(
      <JalaliCalendar {...defaultProps} defaultLevel="decade" minLevel="year" />
    );
    expectLevelsCount([0, 0, 1]);
  ...
  it('calls onYearSelect when year control is clicked', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliCalendar {...defaultProps} level="decade" onYearSelect={spy} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('calls onMonthSelect when month control is clicked', async () => {
    const spy = jest.fn();
    const { container } = render(
      <JalaliCalendar {...defaultProps} level="year" onMonthSelect={spy} />
    );
    await userEvent.click(container.querySelector('table button'));
  ...
  it('supports columnsToScroll', async () => {
    const { rerender } = render(
      <JalaliCalendar {...defaultProps} numberOfColumns={2} columnsToScroll={1} />
    );
    expectHeaderLevels('month', ['April 2022', 'May 2022']);
    await clickNext('month');
    expectHeaderLevels('month', ['May 2022', 'June 2022']);

    rerender(<JalaliCalendar {...defaultProps} numberOfColumns={2} columnsToScroll={2} />);
    expectHeaderLevels('month', ['May 2022', 'June 2022']);
    await clickNext('month');
  ...
  it('supports changing month label format', () => {
    render(<JalaliCalendar {...defaultProps} monthLabelFormat="MM/YY" />);
  ...
  it('supports changing year label format', () => {
    render(<JalaliCalendar {...defaultProps} level="year" yearLabelFormat="MM/YY" />);
  ...
  it('supports changing decade label format', () => {
    render(<JalaliCalendar {...defaultProps} level="decade" decadeLabelFormat="MM/YY" />);
