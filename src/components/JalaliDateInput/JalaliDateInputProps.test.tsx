...
describe('@mantine-datepicker-jalali/JalaliDateInput', () => {
  itSupportsClearableProps(JalaliDateInput, {
    ...defaultProps,
    defaultValue: new Date(2022, 3, 11),
  });
  itSupportsYearsListProps(JalaliDateInput, {
    ...defaultProps,
    defaultLevel: 'decade',
    defaultValue: new Date(2022, 3, 11),
    popoverProps: { opened: true, withinPortal: false, transitionDuration: 0 },
  });
  itSupportsMonthsListProps(JalaliDateInput, {
    ...defaultProps,
    defaultLevel: 'year',
    defaultValue: new Date(2022, 3, 11),
    popoverProps: { opened: true, withinPortal: false, transitionDuration: 0 },
  ...
  it('opens/closes dropdown when input is focused/blurred', async () => {
    const { container } = render(<JalaliDateInput {...defaultProps} />);
  ...
  it('does not open popover if readOnly prop is set', async () => {
    const { container } = render(<JalaliDateInput {...defaultProps} readOnly />);
    expectNoPopover(container);
    await userEvent.tab();
    expectNoPopover(container);
  });
  ...
  it('supports controlled state (dropdown click)', async () => {
    ...
    const { container } = render(
      <JalaliDateInput {...defaultProps} value={new Date(2022, 3, 11)} onChange={spy} />
    );
    await userEvent.tab();
    await clickControl(container, 4);
  ...
  it('allows to clear input value when allowDeselect is set (uncontrolled)', async () => {
    const { container } = render(
      <JalaliDateInput {...defaultProps} allowDeselect defaultValue={new Date(2022, 3, 11)} />
    );
  ...
  it('calls onClick when input is clicked', async () => {
    const spy = jest.fn();
    const { container } = render(<JalaliDateInput {...defaultProps} onClick={spy} />);
    await userEvent.click(getInput(container));
  ...
  it('does not update value on blur if fixOnBlur={false}', async () => {
    const { container } = render(
      <JalaliDateInput {...defaultProps} fixOnBlur={false} defaultValue={new Date(2022, 3, 11)} />
    );
  ...
  it('updates value on blur if fixOnBlur={true}', async () => {
    const { container } = render(
      <JalaliDateInput {...defaultProps} fixOnBlur defaultValue={new Date(2022, 3, 11)} />
    );
